...
async def _build_ai_prompts(params: dict, count: int, has_video: bool = False, has_image: bool = False) -> list:
    ...
    system_prompt = (
        ...
        "🔴 STYLE DIFFERENTIATION RULES (MANDATORY — each prompt MUST feel completely different):\n"
        "   - The CONTENT and CREATIVE APPROACH of each finalPrompt MUST match its assigned styleLabel.\n"
        "   - Do NOT write similar prompts and just swap labels. Each style requires fundamentally different narrative, camera, mood, and pacing.\n\n"
        "   Style-specific creative rules:\n"
        "   1. 痛点解决流 (Pain-Solution): NARRATIVE arc from problem → solution. Open with relatable pain/frustration scene, cold dim lighting, dramatic pause, then product enters as hero. Before/after split-screen transitions. Emotional journey from tension to relief. Camera: slow zoom, documentary style.\n"
        "   2. UGC种草风 (UGC/Vlog): AUTHENTIC amateur feel. Handheld camera with natural shake, selfie/POV angle, no studio lighting. Real environment (bedroom, kitchen). Unboxing moment, genuine surprise reactions. Quick casual cuts between daily use scenes. Casual on-screen text like 'obsessed lately'. BGM: trending lo-fi, NOT cinematic.\n"
        "   3. 产品场景展示 (Product Showcase): PURE product-centric. Smooth orbit/dolly around product, 360° turntable reveal. Elegant particle effects (wind, water droplets, smoke). Macro close-ups on material texture. Premium lighting (rim light, key light). Minimal human presence — product is the star.\n"
        "   4. 暴力测试风 (Durability Test): EXTREME shock value. Open with product being hit/dropped/smashed in first 2 seconds. Slow-motion impact capture, screen shake. Multiple extreme tests (water, drop, stress, bend). Close-up shows ZERO damage after each test. Bold impact font text overlays ('WILL IT SURVIVE?'). Heavy impact sound design.\n"
        "   5. 情绪共鸣流 (Emotional Resonance): WARM intimate tone. Slow dolly with shallow DOF, golden-hour lighting, soft bokeh. Open with relatable daily life moment (tired morning, missing home). Product as emotional companion, not hard sell. Warm desaturated color grading, film grain texture. Gentle piano/acoustic music. Slow deliberate pace, no rush.\n"
        "   6. 极速快剪流 (Hyper-Fast Cuts): MAXIMUM energy, zero patience. Snap zoom fills frame instantly. 0.3-0.5s per cut, product from 5+ angles in 2 seconds. Glitch/beat-sync effects on every transition. Speed ramps (slow-mo 0.3s on impact, back to hyper speed). Flash/strobe light sync. Bold text flashes in sync with beats. Hard-hitting BGM from frame 1.\n"
        "   7. 高端大片风 (Cinematic Luxury): APPLE-level production quality. Slow majestic crane/drone sweep. Product emerges from darkness with volumetric god rays and anamorphic lens flares. Slow, deliberate camera moves — every frame is wallpaper. IMAX-level macro detail on material. Color grading: teal-orange or monochrome gold. Orchestral score. Minimal elegant text, luxury thin font.\n"
        "   8. 搞笑反转风 (Comedy Twist): SETUP → PUNCHLINE structure. Start DEAD SERIOUS (dramatic trailer vibe, intense music, serious voiceover) for 3-5 seconds to build contrast. Then ABSURD comedic twist — product appears in ridiculous context. Deadpan reaction shots, exaggerated double-takes. Record scratch on the twist moment. BGM shifts dramatic → comedic instantly. Meme-ready final frame.\n\n"
